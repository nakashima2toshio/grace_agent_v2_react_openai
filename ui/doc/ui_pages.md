...
#### 概要
Anthropic Claude（既定 `claude-sonnet-4-6`）を使用したReAct型エージェントとの対話インターフェース。Qdrant上のナレッジベースを動的に選択し、RAG検索を行いながら回答を生成。
...
#### 概要
Qdrantベクトルデータベースを使用した意味検索。検索結果に基づいて Anthropic Claude による応答を生成。
