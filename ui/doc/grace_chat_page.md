...
flowchart TB
    ...
    subgraph Planner_Layer["Planner Layer (grace/planner.py)"]
        ...
        PlanGeneration["計画生成<br/>(Claude / JSON)"]
    end
...
sequenceDiagram
    ...
    Planner->>Planner: 5. LLM計画生成 (Anthropic Claude / JSON Schema)
    ...
    loop 各ステップ (Generator)
        ...
        alt rag_search
            ...
            Tools->>Tools: 14. LLM推論 (Anthropic Claude)
        end
...
| **Process** | 1. `_get_available_collections()` — Qdrantから利用可能コレクション一覧を取得<br>2. `estimate_complexity_with_llm(query)` — LLMで複雑度を推定（0.0-1.0）<br>3. `PLAN_GENERATION_PROMPT` にコレクション一覧・クエリを埋め込み<br>4. Anthropic Claude（構造化出力/Tool Use）で `ExecutionPlan` 相当の JSON を生成<br>5. `ExecutionPlan.model_validate_json()` でパース<br>6. `validate_plan_dependencies()` で依存関係を検証<br>7. 計画IDを付与（`create_plan_id()`） |
...
|------|------|
| **Input** | `query: str` — ユーザーの質問文 |
| **Process** | `COMPLEXITY_ESTIMATION_PROMPT` を使い、Anthropic Claude で数値のみを取得（temperature=0.1） |
...
| `anthropic` | >= 0.40 | Anthropic Messages API（LLM: Claude） |
| `google-genai` | >= 0.4 | Gemini Embedding API（埋め込み専用・新SDK） |
...
| `MODEL_NAME` | `ModelConfig.DEFAULT_MODEL`（`claude-sonnet-4-6`） | デフォルトモデル |
