...
uv run python -m chunking.csv_text_to_chunks_text_csv \
  --input-file OUTPUT/cc_news_2per_openai.csv \
...
uv run python qa_qdrant/make_qa_register_qdrant.py \
  --input-file output_chunked/cc_news_2per_openai_chunks.csv \
...
# 最終チャンク全件に強制分割の上限として使う。
# OpenAI Embeddingへ渡す入力を適切な大きさに保つため、余裕を持たせる。
MAX_CHUNK_TOKENS = 512

# チャンク処理で許容するEmbedding入力トークン上限。
...
def _enforce_max_chunk_tokens(chunks: List[str], max_tokens: int) -> List[str]:
    ...
    フォールバックで保全されたブロックには上限がなかった。OpenAI Embeddingへ
    過大な入力を渡さないため、最終チャンク全件に対して上限を強制する。
    """
    enforced: List[str] = []
    split_count = 0
    for chunk in chunks:
...
async def chunks_all_async(
        text: str,
        model: str = ModelConfig.LIGHT_MODEL,
        max_workers: int = 8,
        block_size: int = 1000,
...
async def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default=ModelConfig.LIGHT_MODEL,
        help="使用するLLMモデル"
    )
    parser.add_argument(
        "--workers",
