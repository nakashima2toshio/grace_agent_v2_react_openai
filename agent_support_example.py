...
前提:
- 環境変数に OPENAI_API_KEY（LLM／Embedding 用）を設定
...
# .env から OPENAI_API_KEY を読み込む（未導入でも続行）
try:
...
#
# Qdrantは手動管理されたOpenAIコレクションを参照専用で利用する。
# アプリから作成・登録・更新・削除は行わない。
PROFILES: Dict[str, VerticalProfile] = {
    "gov": VerticalProfile(
        name="自治体",
        collections=["gov_faq_ollama"],
    ...
    "saas": VerticalProfile(
        name="SaaS",
        collections=["saas_api_ollama", "saas_docs_ollama"],
...
def create_run_config(vertical: Optional[str] = None, base_config=None):
    """Create an isolated GRACE config with the vertical Qdrant scope applied."""
...
def execute_support_workflow(
    ...
    """Execute the support workflow and return a structured result.

    The service path is output-independent by default. The CLI adapter opts in
    to the existing human-readable console rendering.
    """
    if not render_output:
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            return execute_support_workflow(
                query=query,
                verbose=verbose,
                use_web=use_web,
                do_action=do_action,
    ...
    if not os.getenv("OPENAI_API_KEY"):
        print("⚠️ OPENAI_API_KEY が未設定です。環境変数に設定してください。", file=sys.stderr)
        return None

    # get_config() is process-global. A run-scoped copy prevents vertical-specific
    # Qdrant collection and prompt settings from leaking across API requests.
    config = create_run_config(vertical, config_override)
    ...
    # ① Plan
    emit("plan_started", query=query, vertical=vertical)
    _banner("① Plan（planner）")
    print(f"❓ 問い合わせ: {query}")
    plan = planner.create_plan(query)
    emit("plan_completed", plan=plan.model_dump(mode="json"))
    print(f"  [plan] {len(plan.steps)} ステップ (complexity={plan.complexity:.2f})")

    # ② Execute（内部 RAG → reasoning）
    emit("execution_started", steps=len(plan.steps))
    _banner("② Execute（executor + tools: 内部RAG）")

    def forward_executor_event(event: Any) -> None:
        if isinstance(event, dict):
            emit("tool_event", event=event)
            return
        step_id = getattr(event, "current_step_id", None)
        statuses = getattr(event, "step_statuses", {})
        ...
        emit(
            ...
            paused=bool(getattr(event, "is_paused", False)),
    ...
    emit(
        "groundedness_completed",
        support_rate=gres.support_rate,
        decided=gres.supported + gres.contradicted,
        contradiction=gres.has_contradiction,
        verified=gres.verified,
    )
    ...
    if decision == "escalate" and use_web and not forced_escalate:
        ...
        emit("web_completed", used=support.used_web, reused=support.web_reused,
             citations=support.citations, agreement=support.source_agreement)
    ...
    if support.decision == "answer" and support.answer:
        ...
        emit("no_info_completed", detected=support.no_info_detected, marker=marker)
    ...
    # ⑦ 応答
    _render(support)
    emit("run_completed", result=support)
    return support
...
def run_support_agent(
    ...
    """Backward-compatible CLI/program adapter with console rendering."""
    return execute_support_workflow(
        query=query,
        verbose=verbose,
        use_web=use_web,
