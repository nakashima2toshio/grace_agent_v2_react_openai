...
本プロジェクトは OpenAI を LLM プロバイダーとするため、同一の呼び出し
インターフェースを保ったまま Responses API を呼び出すアダプターを提供する。
...
Embeddingは別クライアントでOpenAI APIを利用する。本アダプターはLLM生成のみを扱う。
"""
...
class _GenaiCompatResponse:
    ...
    def __init__(self, text: str, usage: Optional[_UsageMetadata] = None, parsed: Any = None):
        self.text = text
        self.parsed = parsed
...
class _OpenAIModels:
    """genaiの`client.models`呼び出しをOpenAI Responses APIへ変換する。"""
    ...
    def generate_content(
        ...
        **_kwargs: Any,
        ...
        if schema is not None and callable(getattr(schema, "model_json_schema", None)):
            response = self._get_client().responses.parse(text_format=schema, **kwargs)
            parsed = getattr(response, "output_parsed", None)
            text = parsed.model_dump_json() if hasattr(parsed, "model_dump_json") else response.output_text
        else:
...
class OpenAIGenaiClient:
    """`.models.generate_content(...)`を提供する遅延初期化OpenAIクライアント。"""

    def __init__(self, default_model: str, api_key: Optional[str] = None):
        self._default_model = default_model
        self._api_key = api_key
        self._client: Any = None
        self.models = _OpenAIModels(self._ensure_client, default_model)
...
def create_chat_client(config: Any = None) -> Any:
    ...
    config.llm.provider に応じて以下を返す:
        ...
        - "openai"          → OpenAIGenaiClient（genai 互換）
    ...
    if provider == "openai":
        return OpenAIGenaiClient(default_model=model)
