...
対象によって**使う仕様書が異なる**。いずれもスキル同梱（`.Codex/skills/...`）で、書く前に該当仕様を**実際に読むこと**。

| 対象 | 使う仕様書 | 中心構造 | 主なドキュメント所在 |
|------|-----------|---------|------------------|
| コード・モジュール（クラス/関数） | `.Codex/skills/grace-agent-docs/a_class_method_md_format.md` | IPO（Input-Process-Output） | `<package>/doc/<module>.md` |
| Streamlit 画面・ページ | `.Codex/skills/grace-agent-docs/a_pages_md_format.md` | 画面レイアウト＋セッション状態＋操作フロー | `ui/pages/doc/<page>.md`（無ければ対象に準ずる） |
| 単体テスト | `.Codex/skills/grace-agent-tests/a_test_md_format.md` | SAE（Setup-Action-Expected） | grace-agent-tests スキル参照 |

> テスト仕様（SAE）は **grace-agent-tests** スキルが担当。本スキルはモジュール（IPO）と画面（ページ）を担当する。
> 開発メモ・サンプルQ&A等の参考資料は `.Codex/skills/grace-agent-docs/a_memo_dev.txt`。

## 1. モジュール仕様（`a_class_method_md_format.md`・IPO形式）— 必読
- 仕様書はスキル同梱 `.Codex/skills/grace-agent-docs/a_class_method_md_format.md`（IPO形式）。**先に読むこと**。
...
## 2. Mermaid 黒背景・白文字（AGENTS.md §7 / 仕様書 §16.5）— 必須
...
## 3. 技術スタック表記の統一（AGENTS.md §9.1）
- LLM = **Anthropic Codex**、既定 `Codex-sonnet-4-6`（軽量 `Codex-haiku-4-5-20251001`）。鍵 `ANTHROPIC_API_KEY`。
...
## 6. 進め方のコツ
- 複数ファイルを最新化するときは **ファイルごとにサブエージェントを並列起動**（各に「**使うフォーマット仕様のパス**（モジュール=`.Codex/skills/grace-agent-docs/a_class_method_md_format.md` / 画面=`.Codex/skills/grace-agent-docs/a_pages_md_format.md`）＋対象ソース＋黒背景Mermaid規約＋スタック表記」を渡す）。
