...
## テストドキュメントのフォーマット仕様
- 単体テストのドキュメントを作成・最新化する場合は、スキル同梱 `.Codex/skills/grace-agent-tests/a_test_md_format.md`（単体テスト ドキュメント フォーマット仕様）に従う。**先に読むこと**。
...
3. **既定値ドリフト（期待値を現行へ）**
   - モデル既定 `gemini-2.0-flash`→`Codex-sonnet-4-6`。
...
`.Codex/skills/grace-agent-tests/a_test_md_format.md` に従う。モジュール仕様（IPO）とは**観点・構成が異なる**ので混同しない。
