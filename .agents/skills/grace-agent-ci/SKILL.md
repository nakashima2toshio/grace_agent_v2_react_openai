...
description: >-
  ...
  when configuring or relying on the Codex/* auto-merge, or when creating
...
- **auto-merge** = `build` 成功後、`head_ref` が `Codex/*` の PR を master へマージ（`gh pr ready` → `gh pr merge --merge`）。`hold` ラベルが付いた PR は対象外。
...
## ブランチ・PR 運用
- 開発は `Codex/<topic>` ブランチ。**ドラフトPRで作成**（auto-merge が Ready 化してマージ）。bootstrap 用に自己マージさせたくない変更（ci.yml 自体など）は `ci/*` 等 `Codex/` 以外の名前にする。
