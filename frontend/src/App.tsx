...
const initial: RunRequest = { query: '', vertical: 'saas', use_web: true, do_action: true, dry_run: true }
...
type StepData = { step_id:number; status:string; confidence?:number; sources?:unknown[]; execution_time_ms?:number }
...
export default function App() {
  ...
  const plan = useMemo(() => events.find(e => e.type === 'plan_completed')?.data.plan as PlanData | undefined, [events])
  const executed = useMemo(() => events.filter(e => e.type === 'executor_state' && e.data.step).map(e => e.data.step as StepData), [events])
  ...
  return <main>
    ...
    {run&&<section className="results"><Title n="03" title="検証と回答" sub="処理結果はイベントとともに更新されます"/>
      <div className="metrics"><Metric k="状態" v={run.state}/><Metric k="Groundedness" v={`${Math.round((run.result?.groundedness??0)*100)}%`}/><Metric k="出典" v={String(run.result?.citations.length??0)}/><Metric k="イベント" v={String(events.length)}/></div>
      <div className="detail-grid">{plan&&<article className="detail-panel"><small>PLAN</small><h3>実行計画</h3><p>複雑度 {plan.complexity.toFixed(2)} ・ {plan.steps.length} steps</p><ol>{plan.steps.map(step=><li key={step.step_id}><b>{step.step_id}. {step.description}</b><span>{step.action}{step.depends_on?.length?` / depends on ${step.depends_on.join(', ')}`:''}</span></li>)}</ol><p className="criteria">成功条件: {plan.success_criteria}</p></article>}{executed.length>0&&<article className="detail-panel"><small>EXECUTE</small><h3>ステップ結果</h3><ol>{executed.map((step,i)=><li key={`${step.step_id}-${i}`}><b>Step {step.step_id} — {step.status}</b><span>confidence {Number(step.confidence??0).toFixed(2)} / sources {step.sources?.length??0} / {step.execution_time_ms??0}ms</span></li>)}</ol></article>}</div>
