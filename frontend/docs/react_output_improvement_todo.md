...
| RO-20 | P1 | DONE | Streamlit／React比較試験を追加する | 実Qdrant、LLM、Web | 同一入力・モデル・実行条件の下、画面固有の検索スコープ差を記録して許容差を満たす |
...
| RO-23 | P2 | DONE | 実行時間とタイムアウト理由を可視化する | event schema、UI | 30秒失敗がtimeout等の理由付きで表示される |
...
| ID | 実施したコード変更 | 完了を証明する検証 |
|---|---|---|
| RO-20 | `scripts/compare_agent_support_surfaces.py`を追加し、gov固定、Web ON、Action ON、dry-run ONで両経路を実行する。Streamlitは全検索可能3072次元コレクション、Reactはgov許可コレクションという実装差を明記する | `streamlit_react_comparison_result.json`でReact=`answer`、Web出典8件、主要4項目一致、Actionなし、比較passed=trueを保存 |
| RO-23 | `StepResult.error_code`と`classify_step_error()`を追加し、timeout/tool_error/cancelled/dependency_error/validation_errorをUIの日本語理由へ変換する | Python分類テスト5件とReactの「タイムアウト」表示テストを通過 |

### 3.3 実比較で追加特定・修正した問題

| 問題 | 原因 | 改修 | 実結果 |
|---|---|---|---|
| Webに8件の有効な出典があるのに`no_information`へ落ちる | GPT-5系の短文分類に`max_output_tokens=10`しか与えず、400または空応答となった | 意図／No-info分類の出力枠を1024へ拡大 | No-info判定APIが成功 |
| Web回答が実質的でもGroundedness JSON崩れで`insufficient_grounding`へ落ちる | Web経路には内部回答と同じ「矛盾なし・出典付き実質回答」の救済がなかった | Web回答にも救済規則を適用し、直後のNo-info安全ゲートで再判定 | 最終`decision=answer`、warning付きで回答維持 |
| govコレクションを検索できない | `gov_faq_ollama`は768次元、検索Embeddingは3072次元であり、検索側の両対応は行わない方針 | コレクションを変更せずWebフォールバックで回答 | Web出典8件を取得。登録・次元変換は未実施 |
...
- [x] 最終ステップ結果に同じ論理Stepが重複しない
- [x] 途中のfailedと最終successを区別できる
- [x] 表示Planと実行された現在Planが一致する
- [x] `escalate`時に不採用回答を通常回答として表示しない
- [x] 通常FAQが`escalate_to_human`承認待ちにならない
- [x] Groundednessが根拠本文を使って計算される
- [x] 判定不能と0%を区別する
- [x] 全実行状態をタイムラインで正しく表現する
- [x] verticalを実行結果から確認できる
- [x] 代表ケースのbackend／Reactテストが成功する
- [x] 同一条件のStreamlit／React比較結果が仕様の許容範囲内である
...
| 1.2 | 2026-07-17 | RO-20／RO-23を完了。実比較で発見した分類出力枠とWeb回答救済を修正し、全完了条件を検証済みに更新 |
