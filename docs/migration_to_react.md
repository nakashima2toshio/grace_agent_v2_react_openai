...
> **Provider移行**: React移行後のAI APIをOpenAIへ全面統一する作業は
> [openai_api_migration_todo.md](openai_api_migration_todo.md) で管理する。
...
6. MVPの状態保存はインターフェースを分離したインメモリ実装とする。再起動耐性・複数worker対応は、`docker-compose/docker-compose.yml` に既存のRedisを使う実装へ置き換える。
...
| P0-06 | P0 | DONE | CLI代表ケースのベースラインを定義する | `eval/vertical/cases/*.jsonl`、`eval/vertical/run.py` | gov／saas／ecでanswer、forced escalate、no-info、action各1件以上を固定 |
| P0-07 | P1 | DONE | 実API利用可能時のCLIベースラインを保存する | `eval/vertical/baselines/agent_support_mock.json` | APIキー未設定・Qdrant未起動のため、answer／no-info／action／escalateのSupportResultと時間をモック基準として保存 |

### Phase 1: サービス分離とデータ契約

| ID | 優先度 | 状態 | TODO | 対象／再利用資産 | 完了条件・検証 |
|---|---|---|---|---|---|
| P1-01 | P0 | DONE | `ActionRequest`、`SupportResult`、`VerticalProfile`を独立schemaへ移す | `services/agent_support_schemas.py` | CLI・APIが同一Pydantic schemaをimportし、既存のpositional ActionRequestも互換維持 |
| P1-02 | P0 | DONE | 実行状態enumと構造化イベントschemaを定義する | `ExecutionState`、`StepResult` | 全処理段階と終端状態を表現しJSON化できる |
| P1-03 | P0 | DONE | `run_support_agent()`を表示非依存サービスへ抽出する | `execute_support_workflow()` | 表示なしで最終結果を返し、event sinkを注入可能。サービス既定runnerとして利用 |
| P1-04 | P0 | DONE | CLI表示をイベント／結果のconsumerとして再実装する | `run_support_agent()`、`_banner()`、`_render()` | 薄いCLI互換adapterで既存引数と主要表示を維持 |
| P1-05 | P0 | DONE | `execute_plan_generator()`をサービス進捗へ接続する | `Executor.execute_plan_with_events()` | executor state、tool event、pause／cancel状態を構造化イベント化 |
| P1-06 | P0 | DONE | 実行ごとに独立したconfigを生成する | `get_config()`、VerticalProfile | 同時実行でallowed collections／prompt addendumが混線しないテストが成功 |
| P1-07 | P0 | DONE | 標準出力依存をログ／event sinkへ置換する | intent、no-info、Web、Action表示 | テスト時にstdout解析なしで状態を検証できる |
| P1-08 | P1 | DONE | Action結果を構造化する | `ActionOutcome` | success、backend、message、action idをSupportResultへ保持 |
| P1-09 | P1 | DONE | CLI互換回帰テストを追加する | `run_support_agent()`公開契約 | `eval/vertical/run.py` が公開契約を変更せず呼び出すことをテスト済み |

### Phase 2: 実行状態・HITL・Action安全境界

| ID | 優先度 | 状態 | TODO | 対象／再利用資産 | 完了条件・検証 |
|---|---|---|---|---|---|
| P2-01 | P0 | DONE | `AgentSupportRunStore`インターフェースを定義する | run、events、pending action、audit | create/get/update/append_event/cancel/resolve_confirmationを提供 |
| P2-02 | P0 | DONE | スレッドセーフなインメモリStoreを実装する | Python標準同期機構 | 並行runの状態とイベントが混線しない |
| P2-03 | P0 | DONE | Action提案時に `pending_confirmation` で停止する | `_decide_action()`、InterventionRequest | 承認前にbackend.executeが一度も呼ばれない |
| P2-04 | P0 | DONE | 承認／却下／修正依頼のサービスAPIを実装する | InterventionResponse | 3応答を区別し、却下時はActionを実行しない |
| P2-05 | P0 | DONE | Action内容ハッシュと承認バージョンを導入する | ActionRequest | 承認後にAction内容が変われば古い承認を拒否 |
| P2-06 | P0 | DONE | Actionの冪等性を実装する | action id、RunStore | 同じ承認POSTを再送してもbackend.executeは1回だけ |
| P2-07 | P0 | DONE | 本人確認結果をAction前に保存する | IdentityVerifier | EC未確認時は確認画面へ進まず有人引継ぎ |
| P2-08 | P1 | DONE | 監査イベントを定義・保存する | InterventionHandler.historyを参考 | 提案、本人確認、承認者、日時、結果、失敗を追跡可能 |
| P2-09 | P1 | DONE | Redis Storeを追加する | `docker-compose/docker-compose.yml` のRedis、RunStore interface | DB 2・専用namespaceへrun／event／承認を保存し、Store再生成後の復元テストが成功 |
| P2-10 | P1 | DONE | pending confirmationの期限切れを実装する | timeout_seconds | 期限切れ承認を拒否しAction未実行 |

### Phase 3: FastAPI

| ID | 優先度 | 状態 | TODO | 対象／再利用資産 | 完了条件・検証 |
|---|---|---|---|---|---|
| P3-01 | P0 | DONE | FastAPIアプリと依存注入を追加する | 既存FastAPI／Uvicorn依存 | health endpointとテストclientが起動 |
| P3-02 | P0 | DONE | `POST /api/agent-support/runs` を実装する | AgentSupportService | query、vertical、use_web、do_action、dry_run、identityを検証しrun_idを返す |
| P3-03 | P0 | DONE | `GET /api/agent-support/runs/{run_id}` を実装する | RunStore | 最新状態、Plan、各評価、最終結果、pending actionを返す |
| P3-04 | P0 | DONE | `GET /api/agent-support/runs/{run_id}/events` SSEを実装する | 構造化イベント | 順序付きevent id、heartbeat、再接続のLast-Event-IDに対応 |
| P3-05 | P0 | DONE | `POST /runs/{run_id}/confirmations` を実装する | HITL service | approve／reject／modifyを受け、権限・version・冪等性を検証 |
| P3-06 | P0 | DONE | `POST /runs/{run_id}/cancel` を実装する | `Executor.cancel()`、run state | 未完了runをcancelledにし、Actionを開始しない |
| P3-07 | P0 | DONE | APIエラー契約を統一する | Pydantic、FastAPI exception handler | validation、not found、conflict、expired、internalをUIが識別可能 |
| P3-08 | P0 | DONE | CORSと秘密情報境界を設定する | FastAPI middleware | 許可originのみ。APIキー・webhook token・identity台帳パスを返さない |
| P3-09 | P1 | DONE | run一覧／履歴APIを追加する | RunStore | 日時、vertical、statusで取得できる |
| P3-10 | P1 | DONE | readiness endpointを追加する | Qdrant、環境変数 | 未設定サービスと起動可否を秘密を漏らさず返す |
| P3-11 | P1 | DONE | OpenAPI schemaからTypeScript型生成を導入する | FastAPI OpenAPI、openapi-typescript | snapshot／生成型をコミットし、pytestとfrontend CIで型ずれを検出 |

### Phase 4: React + TypeScript画面

| ID | 優先度 | 状態 | TODO | 対象／再利用資産 | 完了条件・検証 |
|---|---|---|---|---|---|
| P4-01 | P0 | DONE | Vite + React + TypeScript基盤を追加する | 新規 `frontend/` | dev、test、typecheck、buildの各scriptが成功 |
| P4-02 | P0 | DONE | API clientとSSE clientを実装する | FastAPI契約 | run開始、状態取得、再接続、HITL、cancelを型安全に呼べる |
| P4-03 | P0 | DONE | 問い合わせフォームを作る | CLI引数を画面項目へ対応 | query、gov/saas/ec、Web、Action、dry-run、identityを入力可能 |
| P4-04 | P0 | DONE | ワークフロータイムラインを作る | 状態・イベントschema | ①Plan〜⑥Actionの現在／完了／失敗／skipを表示 |
| P4-05 | P0 | DONE | Planパネルを作る | `plan_completed` event | complexity、steps、depends_on、fallback、success criteriaを表示 |
| P4-06 | P0 | DONE | Execute進捗パネルを作る | `executor_state`、tool events | step status、confidence、sources、時間を段階表示 |
| P4-07 | P0 | DONE | Groundedness／回答ゲートパネルを作る | SupportResult | 支持率、decided、warning、矛盾、強制エスカレーションを表示 |
| P4-08 | P0 | DONE | Web相互検証と出典一覧を作る | citations、agreement、web_reused | Web利用／再利用／一致度と安全なURLリンクを表示 |
| P4-09 | P0 | DONE | HITL確認ダイアログを作る | pending Action、confirmation API | 承認／却下／Action引数の修正依頼とversion更新を実装 |
| P4-10 | P0 | DONE | 最終回答／エスカレーション画面を作る | SupportResult | answerとcitations、または有人引継ぎ理由を明確に表示 |
| P4-11 | P0 | DONE | cancel、retry、APIエラー表示を実装する | API error contract | ユーザーが状態を誤認せず再試行可能 |
| P4-12 | P1 | DONE | 再読込／SSE再接続時の状態復元を実装する | run_id、GET run | localStorageのrun_idからpending confirmationを含め復元するテストが成功 |
| P4-13 | P1 | DONE | 実行履歴画面を作る | run list API | 過去runを一覧表示し、選択した結果へ切替可能 |
| P4-14 | P1 | DONE | アクセシビリティとレスポンシブ対応を行う | WCAG基本項目 | キーボード操作、focus、label、狭幅表示をテスト |

### Phase 5: テスト・デバッグ・動作確認

| ID | 優先度 | 状態 | TODO | 対象／再利用資産 | 完了条件・検証 |
|---|---|---|---|---|---|
| P5-01 | P0 | DONE | サービス単体テストを追加する | 全主要分岐をstub化 | answer／action／Web検証／no-infoのサービス境界を検証 |
| P5-02 | P0 | DONE | config分離・並行実行テストを追加する | gov／saas／ec同時run | profile configが並行実行でも分離され、base configを変更しないことを検証 |
| P5-03 | P0 | DONE | HITL安全テストを追加する | fake backend | 未承認0回、承認1回、重複承認1回、却下0回を検証 |
| P5-04 | P0 | DONE | FastAPI統合テストを追加する | TestClient | run取得、エラー契約、SSE順序再生、confirm→Action completeを検証 |
| P5-05 | P0 | DONE | React component testを追加する | Vitest／Testing Library候補 | 問い合わせ・安全制御の表示を検証 |
| P5-06 | P0 | DONE | E2Eテストを追加する | Vitest + mock API | answer／escalate／HITL修正依頼を検証 |
| P5-07 | P0 | DONE | CLI回帰テストを実行する | 既存pytest／eval | 2026-07-17: 全体で623件成功、20件skip |
| P5-08 | P0 | DONE | Python品質チェックを実行・修正する | ruff、pytest | 対象差分のruffと関連pytestが成功 |
| P5-09 | P0 | DONE | Frontend品質チェックを実行・修正する | lint、typecheck、test、build | 全コマンド成功 |
| P5-10 | P1 | BLOCKED | 実Qdrant／LLM／Webによる代表ケースを確認する | Phase 0 baseline | 2026-07-17: Qdrant／Redis起動・対象collection確認済み。ANTHROPIC_API_KEY・GOOGLE_API_KEY未設定のため、設定後にCLIとReactを比較 |
| P5-11 | P1 | DONE | 障害系を確認する | APIキーなし、Qdrant停止、Web失敗、Webhook失敗 | readiness、依存障害failed、Web失敗escalate、Webhook失敗をモックし、秘密非表示を検証 |
| P5-12 | P1 | DONE | 長時間・再接続・キャンセルを確認する | SSE、RunStore | Last-Event-ID差分再生、long-poll wakeup、cancel後イベント・Action禁止を検証 |

### Phase 6: 文書・運用

| ID | 優先度 | 状態 | TODO | 対象／再利用資産 | 完了条件・検証 |
|---|---|---|---|---|---|
| P6-01 | P1 | DONE | 実装後アーキテクチャ文書を作る | 本TODO、実装コード | API、状態遷移、HITL境界、実ファイルを記載 |
| P6-02 | P1 | DONE | React画面仕様を作る | 実装コンポーネント | 画面領域、状態更新、HITL・復元・履歴の遷移を文書化 |
| P6-03 | P1 | DONE | 開発・起動手順を更新する | README／setup docs | backend、frontend、Qdrantを再現可能な手順で起動 |
| P6-04 | P1 | DONE | 環境変数一覧を更新する | `.env`項目 | 秘密／非秘密、必須／任意、dry-runを明記 |
| P6-05 | P1 | DONE | CIへbackend／frontendチェックを追加する | GitHub Actions | pytest、ruff、typecheck、test、buildをPRで実行 |
| P6-06 | P2 | DONE | Streamlit画面とCLIの将来方針を決める | `agent_rag.py`、`ui/` | CLIは評価用に維持。StreamlitはReact実運用・代表ケース・障害試験完了まで削除しない方針を文書化 |
...
P1のRedis永続Store、履歴、実外部サービス検証、監査強化、CIはMVP直後に実施する。
...
| 状態保存 | interface + インメモリ | 再起動耐性・複数workerは既存composeのRedisへ。SQLite／PostgreSQLは導入しない |
...
| 1.1 | 2026-07-17 | FastAPI、SSE、RunStore、HITL安全境界、React GUI、テスト、CI、起動・アーキテクチャ文書を実装し、TODO状態を実績へ更新 |
